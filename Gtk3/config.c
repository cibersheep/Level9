...
static void update_colour_setting (gchar **colour, ColourSetting *s)
{
	...
	gtk_color_button_get_color (GTK_COLOR_BUTTON (s->button), &c); /*to update gtk_color_button_get_rgba*/
...
/*    gtk_window_set_resizable (GTK_WINDOW(dialog), TRUE);*/ /*CS Seems not needed*/

    /* Top level */

    tabs = gtk_notebook_new ();
    gtk_box_pack_start (
	GTK_BOX (gtk_dialog_get_content_area(GTK_DIALOG(dialog))), tabs, TRUE, TRUE, 0);
